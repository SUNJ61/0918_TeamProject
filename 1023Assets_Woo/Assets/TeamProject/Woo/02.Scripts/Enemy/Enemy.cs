...
public class Enemy : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        fakeCollider = new GameObject().AddComponent<BoxCollider>(); // 가짜 Collider 생성
        fakeCollider.tag = "FlashCol"; // 태그 설정
    ...
    private void FollowPlayertoAttack()
    {
        ...
        if (GameManager.G_instance.isGameover)
        {
            if (!isDead) // 죽지 않은 상태일 때만 게임 오버 처리
            {
                ...
                enemyFlash.OnTriggerExit(fakeCollider); // OnTriggerExit 호출
