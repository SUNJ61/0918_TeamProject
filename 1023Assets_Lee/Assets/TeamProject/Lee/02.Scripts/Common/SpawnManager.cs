...
public class SpawnManager : MonoBehaviour
{
    ...
    public void SetActiveDemonTrue(GameObject candlepos) //촛불이 꺼지면 데몬 함수 호출
    {
        if (candlepos != null)
        {
            ...
            DemonMove move = demon.GetComponent<DemonMove>();
            DemonDamage damage = demon.GetComponent<DemonDamage>();
            DemonAttack attack = demon.GetComponent<DemonAttack>();

            move.Demon_Counter = Demon_IndexCounter;
            damage.Demon_Counter = Demon_IndexCounter;
            attack.Demon_Counter = Demon_IndexCounter;
    ...
    IEnumerator RespawnWait(float Delay, List<Transform> SpawnPoint, GameObject RespawnObj, Transform Standard)
    {
        ...
        InGameSoundManager.instance.ActiveSound(RespawnObj, B_Idle, 6.0f, true, true, true, 1);
    }
