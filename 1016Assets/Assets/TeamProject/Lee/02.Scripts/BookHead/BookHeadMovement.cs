...
public class BookHeadMovement : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        BookHead_Rb = GetComponent<Rigidbody>();
    ...
    public void BookHeadWait()
    {
        StartCoroutine(WaitBookHead());
    }

    IEnumerator WaitBookHead()
    {
        BookHead_agent.speed = 0;
        BookHead_agent.isStopped = true;
    ...
    void OnCollisionEnter(Collision col)
    {
        if (col.gameObject.CompareTag("Player"))
            BookHead_Rb.isKinematic = true;
    }

    void OnCollisionExit(Collision col)
    {
        if (col.gameObject.CompareTag("Player"))
            BookHead_Rb.isKinematic = false;
    }
}
